                    ...
                    <ul v-if="!clapsLegend" class="flex-col justify-start items-start gap-2 inline-flex">
                        <li v-for="topic in topics" :key="topic.id"
                            ...
                            <input :id="`checkbox-${topic.id}`" type="checkbox" :checked="topic.checked"
                                @change="updateChecked(topic.id, topic.name ?? '')"
...
import type {
    components,
    Response as ResponseDashboard
} from '#nuxt-api-party/cmsApiV1'
import type {
    components as componentsCms,
    Response as ResponseCmsApi
} from '#nuxt-api-party/cmsApi'
...
onMounted(async () => {
    // Fetch topics
    try {
        const topicsData = await $cmsApi('/api/v2/topics/', {
            method: 'GET'
        })
        if (topicsData) {
...
const updateChecked = (id: number, name: string) => {
