...
def survey(request):
    context = {"title": "Survey Design", "surveys": SurveyViewSet.GetSurveys()}
...
def survey_detail(request, survey_id):
    ...
    try:
        ...
        context["questions_of_survey"] = QuestionViewSet.GetQuestionsFromSurvey(
            survey_id
        )
...
def question_detail(request, survey_id, question_order):
    ...
    try:
        context["survey_to_display"] = SurveyViewSet.GetSurvey(survey_id)[0]
        context["question_to_display"] = QuestionViewSet.GetOrderedQuestionFromSurvey(
            survey_id, question_order
        )[0]
